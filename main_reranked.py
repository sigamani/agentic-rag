...
def run_assistant(query):
    ...
    chain = (
        RunnableMap({"question": lambda x: x, "context": lambda _: final_docs})
        | (
            lambda inputs: {
                "result": llm.invoke(
                    prompt.format(question=inputs["question"])
                ).content,
                "source_documents": final_docs,
            }
        )
        | (
            lambda outputs: format_answer(
                outputs["result"], outputs["source_documents"]
            )
        )
    )
